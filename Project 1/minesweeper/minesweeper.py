...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # Update neighbors if cell in bounds
                if 0 <= i < self.height and 0 <= j < self.width:
                    neighbors.add((i, j))
        ...
        # update the new_sentence base on current knowledge base
        ...
        # add new sentence into the knowledge base
        ...
        while extra_inference:
            ...
            # 5 add any new sentences to the AI's knoweldge base
            #   if they can be inferred from existing knowledge
            ...
            # If no more new_safes or new_mines, no further infernece check
            extra_inference = not (len(new_safes) == 0 and len(new_mines) == 0)
    ...
    def make_safe_move(self):
        ...
        """
        avaiable = self.safes - self.moves_made
        if avaiable:
            return random.choice(tuple(avaiable))
    ...
    def make_random_move(self):
        ...
        if avaiables:
            return random.choice(tuple(avaiables))
